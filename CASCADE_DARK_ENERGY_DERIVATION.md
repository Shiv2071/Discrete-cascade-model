...
The cascade model is a foundational discrete stochastic system on a finite graph. Cosmology (including dark energy, the Hubble parameter, redshift, and the Friedmann equation) describes phenomena that the cascade model proposes to generate from its own dynamics. This document tests that proposal.
...
**Nothing from cosmology is assumed in steps 1-3.** No Friedmann equation. No CPL ansatz. No fitted Gamma_0. No external H₀. The cascade model has its own dynamics; those dynamics either produce dark energy behavior or they do not. We find out by running the model.

This is the same principle that governed the big bang simulation: we did not fit the explosion regime to cosmological data. We ran the cascade, the explosion emerged, and we then noted the correspondence. The same is done for dark energy.
...
The energy monotonicity theorem (Theorem 3.1, Part I) guarantees:

$$\rho_\beta(n+1) = \rho_\beta(n) - D(n) \leq \rho_\beta(n) \quad \text{a.s., strict when active}$$

This is not an approximation. It is a proved identity.

### I.2 Identifying Today

Step n₀ ("today") is defined as the first step where the system is in the **quiescent regime** and δ(n) has stabilized. This is identified from the simulation output by finding where F(n) ≤ C and δ(n) ≈ δ_Q (approximately constant). The reference values:

$$\beta_0 := \beta(n_0), \quad \delta_0 := \delta(n_0)$$

are purely cascade-internal quantities.
...
- **Φ(n) > 0** for all n where any activity occurs, guaranteed by Theorem 3.1 of Part I: D(n) > 0 whenever active, β(n) > 0
- **Φ(n) → 0** as β → 0 (absorbing state)
- **Three-regime prediction** from the cascade depletion structure:
  - Explosive regime: D(n) ~ Γ₁·β(n)², so δ(n) ~ Γ₁·β(n), which is large → **Φ >> 1**
  - Leakage regime: transitional → **Φ decreasing from >> 1 toward 1**
  - Quiescent regime: D(n) ~ Γ₀·β(n), so δ(n) ~ Γ₀ → **Φ ≈ 1**

The three-regime arc is captured in the shape of Φ(n) vs n.

### II.2 From Φ to w_eff

The cascade master equation (derived in DARK_ENERGY_MAPPING.md Section II, using the cosmological continuity equation after the identification β ↔ ρ_DE) relates Φ to the dark energy equation of state:
...
**The supermartingale theorem (Theorem 3.1, Part I) proves Φ(n) > 0, which proves w_eff(n) > -1 at all n.** This is a structural consequence of the cascade axioms, not a parametric constraint. It holds without approximation, in every realization, with probability one.
...
Cosmological redshift z is defined as 1+z = a₀/a(t_emit). The cascade does not have a metric or scale factor.

However, β(n) plays the role of dark energy density (under the identification), which in cosmology relates to the scale factor. For the cascade in the quiescent regime where δ ≈ Γ₀ = const:

$$\beta(n) = \beta_0 \cdot (1 - \Gamma_0)^{n - n_0}$$

This gives a natural cascade redshift:

$$\boxed{z_c(n) := \frac{\beta(n)}{\beta_0} - 1}$$
...
| Quiescent (today) | ≈ β₀ | ≈ Γ₀ = δ₀ | ≈ 1 | ≈ w₀ = −0.77 | z ~ 0 to 1 |
| Future quiescent | < β₀ | < δ₀ | < 1 | < w₀ | z < 0 |
| Absorbing state | = 0 | = 0 | = 0 | undefined | n → ∞ |

**The cascade prediction for dark energy:** w_eff travels from near 0 (explosive regime, very early universe) through a transitional arc to approximately -0.77 today, then slowly continues toward -1 as β → 0. The phantom divide (w = -1) is never crossed.
...
The `DARK_ENERGY_MAPPING.md` document contains preparatory analytical work:
...
- The analytical LOG and POLY w(z) forms: mean-field approximations of the dynamics, useful for qualitative insight but not the primary simulation-derived result.
- The `fit_desi_bao.py` chi-squared fitting: a preliminary computation that imported cosmological distance formulas and fitted Γ₀ to DESI data. The result (CAS_POLY chi2=8.66 vs CPL chi2=7.99) is noted as a side-product; it is superseded by the simulation-derived approach documented here.
...
3. The structural proof that Φ(n) > 0 implies w_eff(n) > −1 at all n, from Theorem 3.1 of Part I, first stated here.
...
Phi > 0 at all active steps:  True    <- supermartingale theorem (Theorem 3.1, Part I) confirmed
...
1. **Explosive step (z_c ≈ 1.36):** w_eff = -0.584. Dark energy is significantly less negative, consistent with rapid structure formation in the early universe.
...
4. **Phi > 0 throughout** → **w_eff > -1 throughout.** No phantom. Structural theorem (Theorem 3.1, Part I), not a fit.
